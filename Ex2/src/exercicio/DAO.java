...
//import java.sql.*;
	...
	public boolean atualizarUsuario(Cliente usuario) {
		boolean status = false;
		try {  
			Statement st = conexao.createStatement();
	...
	public Cliente[] getUsuarios() {
		...
		try {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM usuario");		
	         if(rs.next()){
	             rs.last();
	             usuarios = new Cliente[rs.getRow()];
	             rs.beforeFirst();

	             for(int i = 0; rs.next(); i++) {
	                usuarios[i] = new Cliente(rs.getInt("codigo"), rs.getString("login"), 
	...
	public Cliente[] getUsuariosMasculinos() {
		Cliente[] usuarios = null;
